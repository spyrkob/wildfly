<xsl:stylesheet version="1.0"
                ...
                xmlns:di="urn:jboss:domain:iiop-openjdk:1.0">
    ...
    <xsl:template match="//*[local-name()='subsystem' and starts-with(namespace-uri(), $iiopNS)]">
        <xsl:copy>
            <xsl:apply-templates select="node()|@*"/>
            <xsl:choose>
                <xsl:when test="not(//*[local-name()='subsystem' and starts-with(namespace-uri(), $iiopNS)]
                                     /*[local-name()='security' and starts-with(namespace-uri(), $iiopNS)])">
