...
class TransactionTimeoutSetupStep implements ServerSetupTask {
    ...
    public void setup(ManagementClient managementClient, String serverId) throws Exception {
        ...
        executeReload(managementClient);

        awaitReload(managementClient);
    }

    @Override
    public void tearDown(ManagementClient managementClient, String serverId) throws Exception {
        setTimeout(managementClient, 300);
    ...
    private void executeReload(ManagementClient managementClient) throws IOException, MgmtOperationException {
        ModelNode op = new ModelNode();
    ...
    private void awaitReload(ManagementClient managementClient) throws java.util.concurrent.TimeoutException {
        final ModelNode op = new ModelNode();
        op.get(ADDRESS).setEmptyList();
        op.get(READ_ATTRIBUTE_OPERATION).set("running-mode");

        RetryTaskExecutor rte = new RetryTaskExecutor<>();
        rte.retryTask(()->{
            final ModelNode mode = ManagementOperations.executeOperation(managementClient.getControllerClient(), op);
            if (!mode.asString().equals("NORMAL")) {
